...
struct StoryBrain {
    ...
    mutating func nextQuestion(сhoice: String) {
        let currentStory = story[questionNumber]
        if сhoice == currentStory.choice1 {
            questionNumber = currentStory.choiceOneNumber
        } else if сhoice == currentStory.choice2 {
            questionNumber = currentStory.choiceTwoNumber
        }
    }
